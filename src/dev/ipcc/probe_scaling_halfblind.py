...
for _ in trange(100):
    ...
    train_subset = np.random.choice(prompts["S1"].unique(), size=n_train, replace=False)
    train_prompts = prompts.loc[(prompts["S1"].isin(train_subset))&(prompts["S2"].isin(train_subset))].sort_values(by=["file", "original_ix"])
    ...
    test_subset = np.random.choice([s for s in prompts["S1"].unique() if s not in train_subset], size=100, replace=False)
    test_prompts1 = prompts.loc[(prompts["S1"].isin(train_subset))&(prompts["S2"].isin(test_subset))]
    test_prompts2 = prompts.loc[(prompts["S1"].isin(test_subset))&(prompts["S2"].isin(train_subset))]
    test_prompts = pd.concat([test_prompts1, test_prompts2]).sort_values(by=["file", "original_ix"])
...
with open(f"{path}/scaling_halfblind/{file}_{n_train}.pkl", "wb") as f:
